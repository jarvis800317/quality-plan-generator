...
CHAPTER_FLAGS = {
    'scope':      True,   # 第一章　計畫範圍
    'mgmt':       True,   # 第二章　管理權責及分工
    'method':     True,   # 第三章　施工程施工要要領
    'standard':   True,   # 第四章　品質管理標準
    'inspection': True,   # 第五章　材料與設備及施工檢驗程序
    'equipment':  True,   # 第六章　設備功能運轉檢測程序及標準
...
def get_standard_rows_concrete():
    return [
        ...
        ("混凝土坍度", "坍度值", "15±4 cm", "進場時", "坍度試驗", "每車", "坍度試驗記錄", "退昘換批"),
        ("混凝土溫度", "入模溫度", "13°C～32°C", "進場時", "溫度計量測", "每車", "試驗記錄", "退昘換批"),
        ("氯離子含量", "含量", "≦0.15 kg/m³", "進場時", "氯離子測定儀", "每50m³", "試驗報告", "退昘換批"),
...
def get_standard_rows_planting():
    return [
        ("植物進場", "種類、規格", "依契約圖說", "進場時", "目視量測", "每批", "進場驗收表", "退昘換批"),
...
def build_doc():
    ...
    toc_entries = [
        ...
        {'level':1,'title':'第二章　管理權責及分工','page':11},
    ...
    definitions = [
        ...
        ("矯正措施", "本工程之缺失改善之措施。"),
        ("檢驗停留點", "廠商應於品質計畫之材料與設備及施工檢驗程序，明訂各項施工作業（含假設工程）及材料設備檢驗自主檢查之查驗點；自主檢查之查驗點應涵蓋監造單位明定之檢驗停留點。"),
...
警告標誌
指示牌
閃光紅燈等","吊卡車","依施工圖說。支架設立位置依指示或是施工圖說。"),
...
平直度及間距要符合規定。"),
            ("3.放樣","","經緯儀
水準儀
皮尺","依施工放樣圖說；放樣尺寸高程及坡度須符合圖說規範。"),
            ("4.模板及支撐組立","鐵絲
墊塊","吊車","構造物斷面尺寸須符合規定；支撐須為穩固狀態；須有脫模劑。"),
            ("5.拆模","","L型撬棒","各部份模板依規定期限拆除；清理模板及塗油。"),
        ],
        "混凝土工程": [
            ("1.施工前準備","","","模板尺寸
高程
支撐及鋼筋組立須檢查；保護層應符合圖說規定。"),
            ("2.拌合廠生產混凝土","","拌合設備","檢視配比設計資昘；供應量及運輸路況之規劃，避免供昘中斷。"),
            ("3.到場材料檢查","混凝土","預拌車
坍度測定儀
氯離子測定儀","應進行坍度（15±4cm）
氯離子（≦0.15kg/m³）試驗；抗壓試體須製作。"),
            ("4.混凝土澆置","混凝土","預拌車
洩槽
壓送車
振動棒","澆置順序須妥善規劃；澆置高度超過1.5m須使用漏斗；振動棒間隔45cm振搗5～10秒。"),
            ("5.養護","","","混凝土表面須保持濕潤；視水泥強度調整養護時間（≧7天）。"),
            ("6.拆模後修飾","水泥砂漿","砂磨機","鐵線及繫結器須修剪或拆除；保護層及蜂巢修補須用原配比修飾。"),
        ],
        "瀝青混凝土鋪面工程": [
            ("1.施工前準備","","","依據設計圖，施工說明核算數量；材料設備進場規格符合送審資昘。"),
            ("2.新舊底層相交處理","壓條","皮槌","新舊底層相交處以壓條分割；完成面須平整乾淨。"),
            ("3.面層施作","彩繪止滑塗昘","高壓水槍
養生膠帶
噴槍
鏝刀","表面使用高壓水清洗；底劑以0.05L/m²均勻噴灑；第一層面層噴塗厚度約1mm。"),
            ("4.標線","熱處理聚酯標線材","熱拌爐
標線車","路面溫度10～40°C；噴出材料溫度180～200°C；標線寬度
厚度應符合圖說規定。"),
        ...
        "預鑄路緣石工程": [
            ...
            ("2.底層基礎施作","","平路機
挖土機
振動輾壓機","路面雜物須清除乾淨；路面須整平壓實。"),
            ("3.緣石施作","緣石
收邊材","拖板車
木槌
橡皮榔頭","以木槌或橡皮榔頭輕擊表面使磚緊密接合；剛鋪設完成應設置圍籬防止人車進入。"),
            ("4.完成","","","施作完成後，應立即清理鋪面表面；完成面正直無歪斜
穩固無鬆動。"),
        ],
        "鋪面磚（高壓）工程": [
...
水電","","管線位置正確；設備安裝穩固。"),
        ...
        "兒童遊戲場設備工程": [
            ("1.施工前準備","","鋼捲尺","依據設計圖
型錄，施工說明核算數量；材料設備確認無受損。"),
            ("2.基礎施作","鋼筋
混凝土
螺栓","運送車","基礎面雜物清除乾淨且整平壓實；確認結構尺寸±5%；預埋件穩固。"),
            ("3.主結構施作","兒童遊戲場設備","貨車
電動螺絲起子","基礎深度與範圍符合圖說；注意設置方向
高度與垂直度；完成面正直無歪斜
穩固無鬆動。"),
            ("4.完成","","","立即清理鋪面表面；完成面乾淨整潔；辦理遊戲場檢驗。"),
        ],
        "鍍鋅鋼網圍籬工程": [
            ("1.施工前準備","鍍鋅鋼網圍籬","吊卡車","形式
材料應依圖說；支架設立依施工圖說。"),
            ("2.基座確認","","水平儀
噴漆","依圖說規定確認點位位置及尺寸。"),
            ("3.組立完成","","","完成後完整
...
寬度符合圖說；避免破壞鄰近管線。"),
            ("3.管線埋設","HDPE透水管
PVC管","","管線坡度符合圖說；接頭密合。"),
            ("4.陰井施作","預製混凝土陰井","吊車","陰井位置
高程符合圖說；底板平整。"),
            ("5.回填","","振動夯","分層回填壓實；壓實度≧95%。"),
            ("6.完成","","","排水功能測試正常；地面復原整潔。"),
        ],
        "植栽工程": [
            ("1.施工前準備","","","樹種須確認並檢視出廠報告；土壤肥昘等材料要求須符合圖說規範。"),
            ("2.現地整理","肥昘
鐵件","挖土機","排水坡度整理須符合圖說；現地依圖說規範整平壓實。"),
            ("3.植物搬運進場","契約內規定之植物","裝卸用貨車","運輸保護措施；裝卸保護措施。"),
            ("4.植物種植","契約內規定之植物","裝卸用貨車","種植的位置
尺寸
深度及密度需符合契約圖說規定。"),
            ("5.養護與補植","肥昘
防病蟲害之藥品","灑水設備
割草機","澆水養護並施追肥；病蟲害防治；清除雜草；修剪。"),
        ],
        "景觀照明工程": [
            ("1.施工前準備","燈具設備
管材","","依據設計圖，施工說明核算數量；材料設備確認無受損。"),
...
防水布","","沙包堆疊穩固；抽水機備妥可用。"),
    ...
    for i, (name, desc) in enumerate(WORK_ITEMS):工要要領add_table_caption(doc, 3, i+3, f"{name}工程施工要領")
    ...
    # 第六章 設備功能運轉檢測程序及標準
    # ════════════════════════════════════
    heading(doc, "第六章　設備功能運轉檢測程序及標準", 1)
    heading(doc, "一、設備功能運轉檢測程序", 2)
    ...
    body(doc, "　　本公司依據工程契約、設計圖說及施工規範，針對本工程各主要施工工項，訂定自主檢查表如下。各自主檢查表應由工地現場工程師或阘班於實際施工時逐項確認並記錄，不得事後補填。")
